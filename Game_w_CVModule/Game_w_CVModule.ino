...
Player *playerList; // Array of all players and dealer :: Player 1, index 0; Player 2, index 1; Player 3, index 2; Dealer, index 3
...
bool cardReadyFlag = false; // flag for when a card is ready to be read from the CV module :: true when ready, false otherwise
/*************************************/
...
volatile int CVinState = 0; // state of the CV modules input
/*************************************/
...
void dealCards() {
  ...
  Serial.println(playerList[3].calculateHandTotal());
  // dealing second round of cards to players
  /*for (int i = 0; i < 3; i++) {
    if (playerList[i].getNumber() != 0) {
